...
menuRoupas.AddItem(menuRoupasItem11 = new UIMenuItem("Outfit Save", "Save your outfit"));
...
///////////////////////////////////////////////////////////////////////////////////////////////////
const menuUotfitSave = new Menu("Outfit Save", "", new Point(50, 50));
let menuUotfitSaveSave = new UIMenuItem("Speichern", "Speicher das Outfit für das Team!");
menuUotfitSave.AddItem(menuUotfitSaveSave);
menuUotfitSave.Visible = false;
todosMenus.push(menuUotfitSave);

menuUotfitSave.ItemSelect.on((item, index, value) => {
    if (item.Text == "Speichern") {
        let mascarasdrawable = parseInt(mascarasItem.SelectedItem.DisplayText);
        let mascarastexture = parseInt(mascarasTextureItem.SelectedItem.DisplayText);
        let chapeusdrawable = parseInt(chapeusItem.SelectedItem.DisplayText);
        let chapeustexture = parseInt(chapeusTextureItem.SelectedItem.DisplayText);
        let jaquetasdrawable = parseInt(jaquetasItem.SelectedItem.DisplayText);
        let jaquetastexture = parseInt(jaquetasTextureItem.SelectedItem.DisplayText);
        let camisasdrawable = parseInt(camisasItem.SelectedItem.DisplayText);
        let camisastexture = parseInt(camisasTextureItem.SelectedItem.DisplayText);
        let calcasdrawable = parseInt(calcasItem.SelectedItem.DisplayText);
        let calcastexture = parseInt(calcasTextureItem.SelectedItem.DisplayText);
        let sapatosdrawable = parseInt(sapatosItem.SelectedItem.DisplayText);
        let sapatostexture = parseInt(sapatosTextureItem.SelectedItem.DisplayText);
        let maosdrawable = parseInt(maosItem.SelectedItem.DisplayText);
        let maostexture = parseInt(maosTextureItem.SelectedItem.DisplayText);
        mp.events.callRemote('saveOutfit', mascarasdrawable, mascarastexture, chapeusdrawable, chapeustexture, jaquetasdrawable, jaquetastexture, camisasdrawable, camisastexture, calcasdrawable, calcastexture, sapatosdrawable, sapatostexture, maosdrawable, maostexture);
    }
});
...
menuRoupas.BindMenuToItem(menuUotfitSave, menuRoupasItem11);

mp.keys.bind(0x71, false, () => {
    if (!mp.players.local.getVariable("aduty")) return;
